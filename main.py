...
def plot(plotDf):
    ...
    ax1.plot(plotDf['start_times'], plotDf['hourly_temp'], color='tab:red', marker='o')
    ax1.tick_params(axis='y', labelcolor='tab:red')
    ax1.set_ylim(40, 80)

    # Format x-axis to show every hour
    ax1.xaxis.set_major_locator(mdates.HourLocator(interval=1))  # Set ticks every hour
    ax1.xaxis.set_major_formatter(mdates.DateFormatter('%H', tz='US/Pacific'))  # Show only hours
    plt.xticks(rotation=45)  # Rotate labels for readability
    ax1.yaxis.set_major_locator(ticker.MultipleLocator(5))  # Set ticks every 5 degrees
    ...
    ax2.plot(plotDf['start_times'], plotDf['hourly_qpf'], color='tab:blue', marker='o')
...
def writeEmailMessage(plotDf):
    ...
    message = f"<p>====== Weather for {formatted_date} ======<p>A high of {int(max_temp.hourly_temp)}F at {max_temp.start_times.strftime("%H")}:00."
...
def sendMessage(plotDf):
    ...
    with open(image_path, 'rb') as img_file:
        ...
        img.add_header('Content-ID', '<image1>')  # This is the reference in the HTML
        msg.attach(img)
